...
#### 背景与定义
FP-Growth 算法由 Han 等人在 2000 年提出，旨在克服传统 Apriori 算法在频繁项集挖掘中的效率问题。Apriori 算法通过生成候选集并多次扫描数据库来发现频繁项集，但当数据集庞大时，候选集的数量可能呈指数级增长，导致计算成本高。FP-Growth 通过构建一个称为 FP-Tree（Frequent Pattern Tree）的紧凑数据结构，消除了候选集生成的需求，从而显著提高了效率。
