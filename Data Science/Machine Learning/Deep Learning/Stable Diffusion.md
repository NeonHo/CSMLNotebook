...
- field: Image synthesis
	...
	- 高分辨率图像合成
		- 复杂自然景观
		- 主流做法：提升 基于相似度模型 的规模
			- 包含数十亿参数的自回归的 transformers[[Transformer]] 。
...
- 大众化的高分辨率图像生成
	...
	- 同时减少训练和采样的计算复杂度
	- 可用性的关键是　减少 DM [[Diffusion Models]]　的计算需求，而无需降低其性能。
- 跳出像素空间，并进入潜在空间
...
- 一个自编码器
	...
	- 这保证了
		- 重构被限制在图像的主流
			...
			- 避免因为只依赖像素空间损失而导致的模糊
- 更准确地说：
	- 对RGB空间中的图像做编码，得到潜在表征。
	- 解码器D从潜在表征中重构出图像。
		- 作者尝试了多种降采样因子
- 为了避免潜在空间的高方差，我们采用了两种不同的正则化。
	- 为了生成的样本能够稳定且一致。
	- 正则化1：
		- 给习得潜在上的标准的norm 施加一个 KL-正则惩罚
		- 与变分自编码器中的VQ-正则类似
			- 在解码器中使用一个向量量化层
			- 模型能够被解释为一个VQGAN
				- 但是有被解码器吸收的量化层
	- 因为我们后续的DM是在潜在空间上的二维结构上工作的，所以我们可以使用相对激进的压缩率并且取得了很好的重构效果。
	- 此前的工作依赖于一个在习得空间上的粗糙的一维排序来自回归地模拟它的分布。
		- 忽略了潜在向量中大量的内在结构。
		- 因此，我们的压缩模型保留了更多关于输入图像的细节。
## 3.2. 潜在扩散模型
- 扩散模型 是 设计为学习数据分布的概率模型
	- 逐渐对一个正常的分布变量 去噪
		- 像是学习一条确定的马尔科夫链的逆过程。[[Section 4 Markov Chain]]
		- 对于图像生成，此前的大多数成功先例都是依赖于 在数据分布的变分低边界的重新加权变体？？？
		- 这镜像了去噪分值匹配。
		- 这些模型都能被理解为 去噪自编码器的加权序列
			- 他们被训练成用于预测 一个 输入图像的被去噪变体
			- 对应的目标被简化为一个损失函数。
- 潜在表征的生成式模型
	- 训练过的感知压缩模型由 编码器和解码器 组成。
		- 因此现在有一个高效的低维潜在空间
			- 其中 高频低维的细节被抽象出来
			- 相比于高维像素空间，这个潜在空间更适合基于相似度的生成式模型。
				- 聚焦于数据中重要的，语义级的bits
				- 在一个低维，高效计算的空间中进行训练。
- 不像此前工作依赖于在高度压缩的离散潜在空间上使用的自回归的基于注意力的transformer模型，我们能够充分利用针对图像的归纳偏置。
	- 从 Conv2D 中构建 潜在的U-Net 的能力
	- 通过使用重新加权的边界， 将目标聚焦在感知最相关的bits上。
	- 神经网络backbone 被实现为一个时间-条件 UNet
	- 因为前向过程固定，训练过程中能够从编码器中高校获取潜在向量。
	- 解码器能轻松从潜在向量中解码到图像空间中。
## 3.3. 条件机制
- 与其他类别的生成模型相似，扩散模型能够模拟条件概率
	- 条件去噪自编码器
	- 为 控制例如文本、语义映射、图生图转换 等的输入合成过程 铺平道路
- DM的生成能力要和其他类型的条件类别标签是一个还有待研究的领域
	- 输入图像的模糊变体
	- 类别标签
- 作者把DM用于一个更加灵活的条件生成器
	- 把UNet backbone 用交叉注意力机制 扩展
	- 对于基于注意力的模型，多模态输入能够高效学习。
	- 为了预处理各种模态，我们引入一个特定领域的编码器来对输入映射一个中间表示。
	- 这些中间表示被映射到U-Net的中间层。
- 基于图像-条件 对，我们学习一个有条件的LDM。
	- 条件机制是灵活的
	- 因为会有特定领域的参数化
