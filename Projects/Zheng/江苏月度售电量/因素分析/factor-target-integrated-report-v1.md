...
本 Markdown 是面向汇总报告的摘要版：每个目标展示最值得解释的因素和典型边界；全量 460 条“目标 × 因素”记录以 CSV 矩阵保存，便于后续筛选、排序、重画图或入稿。
...
这张图只统计已经形成“正向预测证据、线性有效且非线性补充、非线性预测证据”的因素。颜色越深，表示该目标层级下该主题中候选因素越多；它用于发现哪些目标更依赖天气、日历、工业产品产量、库存、商务活动指数或其他公开外生变量。
...
下图把该目标层级的全部因素放在同一张图里。横轴是证据等级编码，只用于阅读排序：越靠右代表越值得优先解释，越靠左代表当前加入后变差或未形成增益；点右侧的 `+`、`-`、`S`、`~`、`?` 分别表示正相关方向、负相关方向、结构性方向、方向较弱/不稳和暂无方向证据。
...
- 建筑业商务活动指数 (%)（建筑业景气）：轻微正向或不稳定；建筑业商务活动指数反映施工、基建、房地产和相关供应链活动景气，可能与第二产业、第三产业和居民侧用电存在间接联动。 边界：国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。

#### 当前不宜作为主输入

- 当前未形成预测增益或加入后变差的因素主要集中在：工业产品产量 18 个；天气 13 个；价格/成本 11 个；服务业景气 9 个；新型负荷/分布式能源 4 个；库存/产销压力 3 个；工业景气/库存指数 2 个；外贸 1 个。
...
| 建筑业商务活动指数 (%) | 建筑业景气 | 正向预测证据 | 负相关倾向 | 建筑业商务活动指数反映施工、基建、房地产和相关供应链活动景气，可能与第二产业、第三产业和居民侧用电存在间接联动。 | 国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。 |
| 日历/节假日/春节错位组合 | 日历/春节 | 正向预测证据 | 结构性影响，方向随工作日/节假日/春节位置变化 | 工作日、节假日和春节错位会改变生产开工、商业活动、居民出行与复工节奏。 | 预测时点已知；但不同产业方向可能不同，不能用同一方向解释所有目标。 |
| 服务业商务活动指数 (%) | 服务业景气 | 正向预测证据 | 正相关倾向 | 服务业商务活动指数反映服务业景气、订单和业务活动扩张收缩，理论上更接近第三产业和居民消费场景。 | 国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。 |
...
| 用户数 | 用户/容量/业务规模 | 正向预测证据 | 正相关倾向 | 用户数和运行容量反映用电主体规模、接入容量和长期需求底座，但需确认预测时点可得性。 | 需确认预测时点可得性，避免同月统计或目标组成口径泄漏。 |
| 综合PMI产出指数 (%) | 综合产出景气 | 正向预测证据 | 弱相关，方向不稳 | 综合 PMI 产出指数反映制造业和非制造业总体产出景气，可作为宏观活动强弱的月度先行/同步指标。 | 国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。 |
| 非制造业商务活动指数 (%) | 非制造业景气 | 正向预测证据 | 弱相关，方向不稳 | 非制造业商务活动指数综合反映服务业和建筑业等非制造部门景气，可能影响第三产业、建筑施工和城市运行用电。 | 国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。 |
...
- 当前未形成预测增益或加入后变差的因素主要集中在：工业产品产量 17 个；天气 7 个；服务业景气 7 个；价格/成本 5 个；新型负荷/分布式能源 3 个；用户/容量/业务规模 3 个；工业景气/库存指数 2 个；工业景气 1 个。
- 这不等于这些因素理论上无用，只表示当前样本、滞后口径和模型下没有形成稳定预测辅助价值。
...
| 建筑业商务活动指数 (%) | 建筑业景气 | 正向预测证据 | 负相关倾向 | 建筑业商务活动指数反映施工、基建、房地产和相关供应链活动景气，可能与第二产业、第三产业和居民侧用电存在间接联动。 | 国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。 |
| 日历/节假日/春节错位组合 | 日历/春节 | 正向预测证据 | 结构性影响，方向随工作日/节假日/春节位置变化 | 工作日、节假日和春节错位会改变生产开工、商业活动、居民出行与复工节奏。 | 预测时点已知；但不同产业方向可能不同，不能用同一方向解释所有目标。 |
| 服务业商务活动指数 (%) | 服务业景气 | 正向预测证据 | 弱相关，方向不稳 | 服务业商务活动指数反映服务业景气、订单和业务活动扩张收缩，理论上更接近第三产业和居民消费场景。 | 国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。 |
| econ_retail_sales_total_社会消费品零售总额_当月同比_百分比 | 消费景气 | 正向预测证据 | 负相关倾向 | 社零反映消费和商业活动，可能影响第三产业、居民消费场景和消费品制造。 | 公开经济数据存在发布时间和累计/同比口径限制，真实预测默认使用滞后项。 |
| 用户数 | 用户/容量/业务规模 | 正向预测证据 | 正相关倾向 | 用户数和运行容量反映用电主体规模、接入容量和长期需求底座，但需确认预测时点可得性。 | 需确认预测时点可得性，避免同月统计或目标组成口径泄漏。 |
| 综合PMI产出指数 (%) | 综合产出景气 | 正向预测证据 | 负相关倾向 | 综合 PMI 产出指数反映制造业和非制造业总体产出景气，可作为宏观活动强弱的月度先行/同步指标。 | 国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。 |
| 非制造业商务活动指数 (%) | 非制造业景气 | 正向预测证据 | 弱相关，方向不稳 | 非制造业商务活动指数综合反映服务业和建筑业等非制造部门景气，可能影响第三产业、建筑施工和城市运行用电。 | 国家数据月度景气指数需按单指标解释；真实预测默认使用滞后或发布日期截断，不能写成 PMI 家族整体结论。 |
...
- 制造业 PMI、非制造业/服务业/建筑业商务活动指数、综合 PMI 产出指数、库存、服务业收入、产品产量和产成品变量应与既有因素平等进入证据矩阵，不应作为附带材料。
...
- 扩展公开景气指标结果：`output/modeling/mvp/d4_9_pmi_extended_factor_analysis/d4_9_single_factor_comparison.csv`
- 扩展公开景气指标方向证据：`output/modeling/mvp/d4_9_pmi_extended_factor_analysis/d4_9_correlation_lag_screen.csv`
